...
class PurchaseRequestList extends Component {
    getDetailPage(data) {
        return (
            <PurchaseRequestDetail row={data} onClickNew={this.onClickNew.bind(this)} saveCallback={this.saveCallback.bind(this)} />
        )
    }
    ...
    state = {
        ...
        masterGrid: {
            columns: [{
                ...
                text: 'PR No',
                style: { maxWidth: '200px', minWidth: '200px', color: '#007bff', textDecoration: 'underline'  },
                events: {
                    onClick: (e, column, columnIndex, row, rowIndex) => {
                        var userId = this.props.storeState.userInfo.userId;
                        var page = row.entityCd === '01' 
                                        ? "Purchase_PR_ETL_Report"
                                        : row.entityCd === '02'
                                            ?  "Purchase_PR_Kimsco_Report" : "Purchase_PR_Testing_Report";
                        var key = row.purchaseId;
            ...
            expandRow: {
                ...
                renderer: row => (
                    <PurchaseRequestListSub row={row} saveCallback={this.saveCallback.bind(this)} pageGb={'Request'} />
                ),
                expanded: [],
                nonExpandable: [],
    ...
    getPurchaseList() {
        ...
        axios.get(this.props.storeState.serviceUrl + "/FiPurchaseService/GetPurchaseList", {
            params: {
                                ...
                                <Form>
                                    ...
                                    <Form.Row>
                                        <div className="col-md-12 text-right">
                                            ...
                                            <Button variant="primary" onClick={this.getPurchaseList.bind(this)} >Search</Button>
                                        </div>
                                    </Form.Row>
                                </Form>
                            </Card.Header>
